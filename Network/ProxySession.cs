...
namespace ProtoTestTool.Network
{
    public class ProxySession : TcpSession
    {
        ...
        public ProxySession(TcpServer server, string upstreamIp, int upstreamPort, ProxyInterceptorPipeline pipeline)
            : base(server)
        {
            _pipeline = pipeline;
            ...
            try
            {
                await foreach (var chunk in channel.Reader.ReadAllAsync())
                {
                    ...
                    while (accumulator.Length > 0)
                    {
                        ...
                        var consumed = ScriptGlobals.Codec.TryDecode(ref span, out var packet);
                        if (consumed <= 0)
                            break;
                        ...
                        await _pipeline.InterceptorCall(context);
                        ...
                        var dataToSend = context.Bypass && context.Raw.Length > 0
                            ? context.Raw
                            : ScriptGlobals.Codec.Encode(context.Packet);
